    ...
    def test_network(self, test_list, test_path, path_save_model):
        ...
        for line in lines:
            ...
            with torch.no_grad():
                ...
                if model_id in enrollments:
                    ...
                    if predicted_phrase_label in enrollment_data:
                        ...
                        score = torch.mean(torch.matmul(speaker_emb, enrollment_embedding.T)).detach().cpu().numpy()
                    else:
                        score = 0  # Handle missing phrase labels appropriately
                else:
                    score = 0
            ...
            if trial_type:
                ...
                if speaker_score is not None and phrase_score is not None:
                    if (score == speaker_score) and (phrase_score == 1):
                        score = 1
                    else:
                        score = 0
