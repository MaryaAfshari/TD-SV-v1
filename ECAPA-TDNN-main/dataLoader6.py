...
def load_phoneme_alignments(textgrid_file):
    tg = textgrid.TextGrid.fromFile(textgrid_file)
    phoneme_intervals = tg.getFirst("phones")
    phonemes = []
    for interval in phoneme_intervals:
        phonemes.append((interval.mark, interval.minTime, interval.maxTime))
    return phonemes

class train_loader(object):
    def __init__(self, train_list, train_path, musan_path, rir_path, num_frames, textgrid_path, **kwargs):
        ...
        for index, line in enumerate(lines):
            try:
                ...
                if phrase_label_str == "FT":
                    continue  # Skip lines with phrase label "FT"
    ...
    def __getitem__(self, index):
        ...
        textgrid_file = os.path.join(self.textgrid_path, os.path.basename(file_name).replace(".wav", ".TextGrid"))
        phoneme_alignments = load_phoneme_alignments(textgrid_file)

        return torch.FloatTensor(audio[0]), self.speaker_labels[index], self.phrase_labels[index], phoneme_alignments
