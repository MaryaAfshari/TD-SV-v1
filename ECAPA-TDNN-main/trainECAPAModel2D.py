...
while(1):
    ...
    if epoch > 6: # I should change it later if I want to train from the base ........5.6.5024
    ...
    if epoch % args.test_step == 0:
        ...
        EER, minDCF, epoch_scores, epoch_labels = s.test_network(test_list=args.eval_list, test_path=args.eval_path, path_save_model=args.path_save_model)
        EERs.append(EER)
...
# Save the best scores to answer.txt
best_epoch_index = np.argmin(EERs)
best_scores = scores[best_epoch_index]

answer_file_path = os.path.join(args.save_eval_path, 'answer.txt')
with open(answer_file_path, 'w') as f:
    for score in best_scores:
        f.write(f"{score}\n")

print(f"Best scores saved to {answer_file_path}")
