...
phrases_to_phonemes = {
    ...
    "FT": ["U", "N", "K"]  # Special token for unknown phrases
}

FT_LABEL = -1  # Special numeric value for "FT" labels
...
def inspect_train_list_for_ft(train_list):
    ...
    It also counts the number of such lines and calculates the percentage.
    """
...
class train_loader(Dataset):
    def __init__(self, train_list, train_path, musan_path, rir_path, num_frames, **kwargs):
        ...
        ft_percentage = inspect_train_list_for_ft(train_list)

        # Ask user whether to continue
        continue_loading = input(f"'FT' phrase labels account for {ft_percentage:.2f}% of the train list. Do you want to continue? (y/n): ")
        if continue_loading.lower() != 'y':
            raise Exception("User chose to stop the data loading process.")
        ...
        for index, line in enumerate(lines):
            try:
                ...
                if phrase_label == "FT":
                    phrase_label = FT_LABEL
                
                self.speaker_labels.append(speaker_label)
                self.phrase_labels.append(int(phrase_label))  # Ensure phrase_labels are integers
    ...
    def __getitem__(self, index):
        ...
        elif self.phrase_labels[index] == FT_LABEL:
            phoneme_ids = torch.LongTensor([ord(ch) for ch in 'UNK'])
        else:
...
def collate_fn(batch):
    ...
    # Ensure phrase_labels are converted to integer tensor if possible
